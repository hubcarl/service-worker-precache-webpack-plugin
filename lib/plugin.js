...
class ServiceWorkerWebpackPlugin {
  ...
  apply(compiler) {
    ...
    compiler.plugin('after-emit', (compilation, callback) => {
      ...
      const swManifestFileName = this.options.prefix ? [this.options.prefix, 'manifest.json'].join('-') : 'manifest.json';
      ...
      const swManifest = {
        config: { 
          localPublicPath : this.options.localPublicPath,
      ...
      Promise.all(promises).then(result => {
        result.forEach(item => {
          swManifest[item.source] = localPublicPath + item.target;
        });
        this.writeServiceWorkerManifest(compiler, swManifestFilePath, swManifest);
